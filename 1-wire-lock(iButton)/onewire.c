...
ow_err ow_tx_reset(ow_Pin *pin)
{
	...
	if (!(*pin->line_read)()){
		uart_print("Line can't recover 2!\n");	/* if still logic 0 */
		return OW_ELINEBREAK;
	}
...
ow_err __ow_cmd_readrom(ow_Pin *pin, uint8_t *rombytes, uint8_t *devcrc, bool resetfirst, bool fastfail)
{
	...
	if ((err != OW_EOK) && fastfail){
		uart_print("failed to transmit READROM command!\n");
		return err;}
	...
	for (uint8_t i = 0; i < 7; i++) {
		...
		soft_delay_us(25);
		if ((err != OW_EOK) && fastfail){
			uart_print("Can't get any data from the device!\n");
			return err;
			}
	}
	...
	if ((err != OW_EOK) && fastfail){
		uart_print("Can't get CRC from the device!\n");
		return err;
	}
	...
	if (readcrc != calccrc || readcrc == 0xff){
		uart_print("\nCRC mismatch!\n");
